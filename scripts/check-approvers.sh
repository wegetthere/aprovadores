...
get_ruleset_id() {
    ...
    log_info "Determinando ruleset baseado na branch: ${target_branch}"
...
get_required_approvals_count() {
    ...
    log_info "Obtendo número mínimo de aprovações do ruleset..."
    ...
    required_count=$(echo "${ruleset_data}" | jq -r '.rules[] | select(.type == "pull_request") | .parameters.required_approving_review_count')
    
    if [[ "${required_count}" == "null" || -z "${required_count}" ]]; then
        log_error "Não foi possível obter o número mínimo de aprovações do ruleset"
        exit 1
    fi
    
    log_info "Aprovações mínimas necessárias: ${required_count}"
    echo "${required_count}"
}
...
main() {
    ...
    # Obtém ruleset ID
    local ruleset_id
    if ! ruleset_id=$(get_ruleset_id "${target_branch}"); then
    ...
    local required_count
    required_count=$(get_required_approvals_count "${ruleset_id}")
